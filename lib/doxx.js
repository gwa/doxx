...
module.exports = function(options) {
	...
	function getPages(pagedir) {
		var i,
			pages = [],
			source = path.resolve(process.cwd(), pagedir),
			mdfiles  = dir.collectFiles(source, {ending:'md'});
		mdfiles.forEach(function(file) {
			var path = pagedir + '/' + file,
				content,
				name;
			if (fs.existsSync(path)) {
				...
				pages.push({
					name:       name,
					targetName: 'page-' + file + '.html',
					readme:     marked(content),
					dox:        [],
					symbols:    []
				});
			}
		...
		for (i = pages.length - 1; i >= 0; i--) {
			files.unshift(pages[i]);
		};
	}
	if (options.pages) {
		getPages(options.pages);
	}
